...
from loader.kdata_multi_echo_CBIC_075_prosp import kdata_multi_echo_CBIC_075_prosp
from utils.data import load_nii, save_nii, r2c, save_mat, readcfl, memory_pre_alloc, torch_channel_deconcate, torch_channel_concate, Logger, c2r_kdata
...
if __name__ == '__main__':
    ...
    ncoil = 8
    nrow = 258
    nrow_interpolate = 412
    ncol = 112
    nslice = 320
    nslice_interpolate = 512
    lambda_dll2 = 1e-3
    ...
    parser.add_argument('--num_ft_subs', type=int, default=8)  # number of new cases used for fine-tuning to 0.75 mm from 1 mm pre-training
    ...
    parser.add_argument('--normalization', type=float, default=1.8)  # 1.8 was used for FA=15
    ...
    rootName = '/data3/Jinwei/QSM_raw_CBIC_075'  # GPU1
    ...
    if opt['flag_train'] == 1:
        ...
        while epoch < niter:
            ...
            if Validation_psnr[-1] == max(Validation_psnr):
                torch.save(netG_dc.state_dict(), rootName+'/'+opt['weights_dir']+'/bcrnn={}_loss={}_K={}_loupe={}_ratio={}_solver={}_unet={}_num_ft_subs={}.pt' \
                .format(opt['bcrnn'], opt['loss'], opt['K'], opt['loupe'], opt['samplingRatio'], opt['solver'], opt['flag_unet'], opt['num_ft_subs']))
            torch.save(netG_dc.state_dict(), rootName+'/'+opt['weights_dir']+'/bcrnn={}_loss={}_K={}_loupe={}_ratio={}_solver={}_unet={}_num_ft_subs={}_last.pt' \
            .format(opt['bcrnn'], opt['loss'], opt['K'], opt['loupe'], opt['samplingRatio'], opt['solver'], opt['flag_unet'], opt['num_ft_subs']))  
    ...
    if opt['flag_train'] == 0:
        ...
        weights_dict = torch.load(rootName+'/'+opt['weights_dir']+'/bcrnn={}_loss={}_K={}_loupe={}_ratio={}_solver={}_unet={}_num_ft_subs={}.pt' \
                .format(opt['bcrnn'], opt['loss'], opt['K'], opt['loupe'], opt['samplingRatio'], opt['solver'], opt['flag_unet'], opt['num_ft_subs']))
        weights_dict['lambda_lowrank'] = torch.tensor([lambda_dll2])
        netG_dc.load_state_dict(weights_dict)
        netG_dc.to(device)
        ...
        with torch.no_grad():
            ...
            if opt['interpolate'] == 1:
                ...
                kdata = np.pad(kdata, ((0, 0), (0, 0), (ncol//2, ncol//2), (0, 0)))
                ...
                kdata = np.fft.fftshift(np.fft.fftn(Recons_, axes=(0, 1, 2)), axes=(0, 1, 2))
                kdata = np.pad(kdata, (((nslice_interpolate-nslice)//2, (nslice_interpolate-nslice)//2), 
                                       ((nrow_interpolate-nrow)//2, (nrow_interpolate-nrow)//2), (ncol//2, ncol//2), (0, 0)))
                Recons_ = np.fft.ifftn(np.fft.ifftshift(kdata, axes=(0, 1, 2)), axes=(0, 1, 2))  # (nslice, nrow, ncol, necho)
            ...
            if opt['interpolate'] == 0 and opt['padding'] == 0:
                if opt['lambda1'] == 1:
                    ...
                    save_mat(rootName+'/results_ablation2/iField_bcrnn={}_loupe={}_solver={}_sub={}_ratio={}_num_ft_subs={}.mat' \
                        .format(opt['bcrnn'], opt['loupe'], opt['solver'], opt['test_sub'], opt['samplingRatio'], opt['num_ft_subs']), 'Recons', Recons_)
            ...
            if opt['padding'] == 0:
                if opt['interpolate'] == 0:
                    os.system('/data2/Jinwei/MEDI.r125/medin ' + rootName + '/results_QSM/iField.bin' 
                            ...
                            + ' -m ' + rootName + '/results_QSM/Mask_sub5_loupe=-2_noft' # for ft 
                            + ' --CSF ' + ' -of QR')
            ...
            if opt['lambda1'] == 1:
                ...
                sio.savemat(rootName+'/results_ablation2/QSM_bcrnn={}_loupe={}_solver={}_sub={}_ratio={}_num_ft_subs={}.mat' \
                    .format(opt['bcrnn'], opt['loupe'], opt['solver'], opt['test_sub'], opt['samplingRatio'], opt['num_ft_subs']), adict)
