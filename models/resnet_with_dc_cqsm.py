...
class Resnet_with_DC2(nn.Module):
    ...
    def __init__(
        ...
        if self.flag_2D == 1:
            ...
            if self.flag_loupe == 2 or self.flag_loupe == -2:
                ...
                temp = torch.zeros(self.necho, self.nrow)  # (necho, nrow)
    ...
    def forward(self, kdatas, csms, csm_lowres, masks, flip, test=False, x_input=None):
        ...
        x_start = x
        # xtest=[]
        # xtest.append(x.cpu().detach())
        # xtest = np.squeeze(r2c(np.concatenate(xtest, axis=0), 1))
        # xtest = np.transpose(xtest, [1, 2, 0])
        # save_mat('/data3/Jiahao/cardiacQSM/results_ablation2/backwards_me_test.mat', 'x_start', xtest)
        # print('save the backward_multiEcho output!')
        ...
        # Deep ADMM
            ...
            if self.flag_BCRNN == 0:
                ...
                # xtest=[]
                # xtest.append(Xs.cpu().detach())
                # # xtest = np.squeeze(r2c(np.concatenate(xtest, axis=0), 1))
                # # xtest = np.transpose(xtest, [1, 2, 0])
                # save_mat('/data3/Jiahao/cardiacQSM/results_ablation2/back_forwards_me_test.mat', 'xtest', xtest)
                # print('save the output after CG!') 
                return Xs
