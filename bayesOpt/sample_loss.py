...
def gen_pattern(a, b, r_spacing=3, sampling_ratio=0.1):
    M_c = 256/2;  # center of kspace
    N_c = 192/2;  # center of kspace
    ...
    xbar = np.mean(p_pattern)
    r = sampling_ratio / xbar
    beta = (1-sampling_ratio) / (1-xbar)
    p_pattern = (r<=1) * p_pattern * r + (1-(r<=1)) * (1 - (1-p_pattern) * beta)
    # keep the center square for calibration
    p_pattern[M_c-13:M_c+12, N_c-13:N_c+12] = 1
    return p_pattern

def recon_loss(params, data_loader, device):
    print('a = {0}, b = {1}'.format(params[0], params[1]))
    p_pattern = gen_pattern(a=params[0], b=params[1], r_spacing=3, sampling_ratio=0.1)
    model = DC_ST_Pmask(input_channels=2, filter_channels=32, lambda_dll2=1e-4, 
                        ...
                        flag_TV=1, K=10, rescale=True, lag_fix=1, pmask_BO=p_pattern)
    ...
    return np.mean(np.asarray(metrices_test.PSNRs))
